// integration_test/add_product_flow_bengali_test.dart
//
// EXPLORATORY VARIANT of add_product_flow_test.dart - selects Bengali instead
...
// add_product_flow_hindi_test.dart, run for the third of the four non-English
// supported languages.
//
...
// target, so every call short-circuits. Selecting Bengali makes
// sourceLanguageCode 'bn-IN' (AppLanguage.bengali.sarvamCode), genuinely
...
//
// 1. Language card tap - $('Bengali'), not native-script matching or index.
//    Confirmed in language_screen.dart: _LanguageCard renders lang.nativeName
//    AND lang.englishName as two SEPARATE Text widgets. For Bengali
//    (app_language.dart: bengali('bn', 'বাংলা', 'Bengali')), englishName is
//    the literal string 'Bengali' - present verbatim in the widget tree as
...
//    $('Bengali') is exactly as robust as $('English')/$('Hindi') were.
//
// 2. Continue button tap - $('এগিয়ে যান') (copied verbatim from
//    app_strings.dart's 'continueBtn'/'bn' entry, not retyped, to avoid
...
//    just selected - once the Bengali card is tapped (calling
//    languageProvider.notifier.setLanguage(bengali)), the button's text
//    immediately switches to Bengali. Every other tap in the sequence
...
//
// SANITY CHECK - WHERE DOES THE BENGALI TEXT ACTUALLY COME FROM: confirmed in
...
// from the languageCode passed in ('bn' here). So the AI-generated
// title/category/description come back FROM GEMINI ALREADY IN BENGALI - not
// in English first and then translated. The Sarvam translation step exists
// specifically to convert this Bengali content back to English for the
...
void main() {
  patrolTest(
    'home add product bengali language publish completes successfully',
    ($) async {
      ...
      // --- Language screen: Bengali instead of English - see header note ---
      ...
      await $('Bengali').scrollTo();
      await $.tap($('Bengali'));
      await $.pumpAndSettle();

      // Continue button text is now Bengali (tr('continueBtn') reactively
      // re-rendered after selecting Bengali) - see header note.
      ...
      // --- Step 3: Review ---
      // Expect Bengali script here - see header note on where the AI output
      ...
      // in Bengali at this point, so toEnglish()'s Sarvam calls should
      // actually hit the network (sourceLanguageCode 'bn-IN' !=
      ...
      // ignore: avoid_print
      print('--- ADD PRODUCT FLOW METRICS (BENGALI) ---');
      metrics.forEach((step, duration) {
        // ignore: avoid_print
        print('$step: ${duration.inMilliseconds}ms');
      });
      // ignore: avoid_print
      print('--- ADD PRODUCT AI OUTPUT (BENGALI) ---');
      // ignore: avoid_print
      print('title: $title');
      // ignore: avoid_print
...
//
//   patrol test --target integration_test/add_product_flow_bengali_test.dart -d <device-id>
//
...
//   adb -s <device-id> logcat -d | grep "PUBLISH TIMING BREAKDOWN\|ADD PRODUCT FLOW METRICS (BENGALI)\|ADD PRODUCT AI OUTPUT (BENGALI)" -A6
