// integration_test/add_product_flow_tamil_test.dart
//
// EXPLORATORY VARIANT of add_product_flow_test.dart - selects Tamil instead of
...
// add_product_flow_hindi_test.dart, run for the fourth and last of the
...
// target, so every call short-circuits. Selecting Tamil makes
// sourceLanguageCode 'ta-IN' (AppLanguage.tamil.sarvamCode), genuinely
...
//
// 1. Language card tap - $('Tamil'), not native-script matching or index.
//    Confirmed in language_screen.dart: _LanguageCard renders lang.nativeName
//    AND lang.englishName as two SEPARATE Text widgets. For Tamil
//    (app_language.dart: tamil('ta', 'தமிழ்', 'Tamil')), englishName is the
//    literal string 'Tamil' - present verbatim in the widget tree as its own
//    Text widget regardless of the app's currently active language (these
//    are fixed enum properties, not translated via tr()). So $('Tamil') is
//    exactly as robust as $('English')/$('Hindi') were.
//
// 2. Continue button tap - $('தொடரவும்') (copied verbatim from
//    app_strings.dart's 'continueBtn'/'ta' entry, not retyped, to avoid
...
//    just selected - once the Tamil card is tapped (calling
//    languageProvider.notifier.setLanguage(tamil)), the button's text
//    immediately switches to Tamil. Every other tap in the sequence (signup,
//    OTP, add-product wizard) is key-based (Key(...)), never text-based, so
//    nothing else needed changing - confirmed during the Hindi variant's
//    investigation and unchanged since.
//
// SANITY CHECK - WHERE DOES THE TAMIL TEXT ACTUALLY COME FROM: confirmed in
...
// from the languageCode passed in ('ta' here). So the AI-generated
// title/category/description come back FROM GEMINI ALREADY IN TAMIL - not in
// English first and then translated. The Sarvam translation step exists
// specifically to convert this Tamil content back to English for the
...
void main() {
  patrolTest(
    'home add product tamil language publish completes successfully',
    ($) async {
      ...
      // --- Language screen: Tamil instead of English - see header note ---
      // Defensive scrollTo() before the tap - this is the fix for the
      // reproducible "found but not hit-testable" failure root-caused via
      // uiautomator dump: Tamil is the 5th/last card, and on this device's
      // screen size the ListView's viewport (1178px) is ~88px too short to
      // fit all 5 cards (242px each + separators) at rest, clipping Tamil's
      // card to a ~53px sliver - the englishName "Tamil" text sits lower in
      // that row's layout and lands entirely inside the clipped, non-hit-
      // testable region. scrollTo() scrolls the ListView (auto-detected as
      // the nearest Scrollable ancestor) until the target is fully visible
      // before the tap is attempted, matching what a real user would do
      // instinctively but Patrol's plain tap() does not do on its own.
      await $('Tamil').scrollTo();
      await $.tap($('Tamil'));
      await $.pumpAndSettle();

      // Continue button text is now Tamil (tr('continueBtn') reactively
      // re-rendered after selecting Tamil) - see header note.
      await $.tap($('தொடரவும்'));
      await $.pumpAndSettle();
      ...
      // --- Step 3: Review ---
      // Expect Tamil script here - see header note on where the AI output
      ...
      // in Tamil at this point, so toEnglish()'s Sarvam calls should actually
      // hit the network (sourceLanguageCode 'ta-IN' != targetLanguageCode
      ...
      // ignore: avoid_print
      print('--- ADD PRODUCT FLOW METRICS (TAMIL) ---');
      metrics.forEach((step, duration) {
        // ignore: avoid_print
        print('$step: ${duration.inMilliseconds}ms');
      });
      // ignore: avoid_print
      print('--- ADD PRODUCT AI OUTPUT (TAMIL) ---');
      // ignore: avoid_print
      print('title: $title');
      // ignore: avoid_print
...
//
//   patrol test --target integration_test/add_product_flow_tamil_test.dart -d <device-id>
//
...
//   adb -s <device-id> logcat -d | grep "PUBLISH TIMING BREAKDOWN\|ADD PRODUCT FLOW METRICS (TAMIL)\|ADD PRODUCT AI OUTPUT (TAMIL)" -A6
